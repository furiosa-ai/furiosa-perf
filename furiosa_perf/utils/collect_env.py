...
class SystemDetector:
        ...
        try:
            ...
            for device in devices:
                ...
                if npus['name'] != str(device_info.arch()).upper():
                    ...
                    npus['memory'] = "{} GiB".format(round(sum([mem_block.total_bytes() for mem_block in memory_utilization.dram().memory()])
                        / (1024 * 1024 * 1024), 2))
    ...
    def _get_gpu_info() -> dict[str, Any]:
        ...
        try:
            rc, out, err = run_lambda("nvidia-smi --query-gpu=name,memory.total,driver_version,vbios_version --format=csv,nounits")
            if rc == 0:
