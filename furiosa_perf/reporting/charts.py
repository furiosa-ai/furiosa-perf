...
from furiosa_perf.reporting.schemas import HyperbolicModel
        ...
        data=[
            go.Table(
                ...
                header=dict(
                    ...
                    font=dict(
                        family=table_chart_theme.font_family, size=table_chart_theme.header_font_size, color="#ffffff"
                    ),
                    line=dict(width=0),
                ...
                cells=dict(
                    ...
                    font=dict(
                        family=table_chart_theme.font_family, size=table_chart_theme.cell_font_size, color="#ffffff"
                    ),
                    line=dict(width=0),
                    line_color="rgba(255,255,255,0.0)",
                ),
                meta=dict(role="rngd_table"),
            )
        ]
    )
...
def plot_line_chart(
    ...
    line_chart_fig = px.line(
        ...
        y=y_axis,
        color="device",
        color_discrete_map={device: get_device_color(device, RNGD_IDX) for device in sorted(df["device"].unique())},
        markers=True,
    )
    ...
    line_chart_fig.for_each_trace(lambda tr: tr.update(line=dict(dash="dot")) if "TARGET" in (tr.name or "") else None)
    ...
    line_chart_fig.update_layout(
        ...
        plot_bgcolor="#000000",
        margin=dict(l=40, r=line_chart_theme.panel_w + 30, t=30, b=30),
        legend=dict(
            traceorder="grouped",
            tracegroupgap=12,
            font=dict(
                family=line_chart_theme.font_family, size=line_chart_theme.font_size, color=line_chart_theme.font_color
            ),
...
def plot_interactive_user_chart(
    ...
    for device, group in sorted(df.groupby("device")):
        g = group.sort_values("TPS/User")
        a, b = HyperbolicModel.fit_hyperbolic_model(g["Concurrent"].to_numpy(), g["TPS/User"].to_numpy())
        init_supported = HyperbolicModel.max_conc_from_fit(a, b, INIT_SLO)
        ...
        entire_chart.add_trace(
            go.Bar(
                ...
                legendgroup=device,
                showlegend=False,
                marker_color=get_device_color(device, RNGD_IDX),
            ),
            row=2,
            col=1,
        )
    ...
    entire_chart.update_layout(
        ...
        meta=dict(labels=devices, fitA=a_list, fitB=b_list),
        legend=dict(
            orientation="v",
...
def plot_rack_performance_chart(df: pd.DataFrame) -> go.Figure:
    ...
    for device, group in sorted(df.groupby("device")):
        ...
        rack_performance_chart.add_trace(
            go.Scatter(
                ...
                name=device_name,
                line_shape="hv",
                line=dict[str, int | str](width=3, dash=dash, color=get_device_color(device, RNGD_IDX)),
            )
        )
        ...
        device_metadata.append(
            {
                ...
                "color": get_device_color(device, RNGD_IDX),
            }
        )
    ...
    rack_performance_chart.update_layout(
        ...
        font=dict(family="Favorit-medium, system-ui", color="#ffffff", size=14),
        paper_bgcolor="#000000",
        plot_bgcolor="#000000",
        width=750,
        height=620,
        legend=dict(
            ...
            font=dict(family="Favorit-medium, system-ui", color="#eeeeee", size=14),
            bgcolor="rgba(0,0,0,0)",
            bordercolor="rgba(0,0,0,0)",
        ),
        margin=dict(r=40, l=40, t=30, b=30),
        meta=dict(device_metadata=device_metadata, max_rack_kw=36000),
    )
